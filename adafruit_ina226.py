...
`adafruit_ina226`
================================================================================
CircuitPython driver for the TI INA226 current and power sensor
* Author(s): Bryan Siepert (for INA260)
Modified by: Kirk Franks to support INA226
...
_REG_AMPS        = const(0x04)  # CURRENT REGISTER (R)
_REG_CALIBRATION = const(0x05)  # FULL SCALE CALIBRATION REGISTER (R/W)
...
class INA226:
    """Driver for the INA226 power and current sensor.

    :param ~busio.I2C i2c_bus: The I2C bus the INA226 is connected to.
    ...
    def __init__(self, i2c_bus, address=0x40):
        ...
        if self._device_id != self.INA226_ID:
            raise RuntimeError(
                "Failed to find INA226 ID, read {} while expected {}"
                " - check your wiring!".format(self._device_id, self.INA226_ID)
            )
    ...
    """The mode that the INA226 is operating in. Must be one of
